...
class ExtractCode(BaseModel):
    ...
    def run(self):
        """
        Thực thi việc trích xuất và chạy mã lệnh từ nhiệm vụ.

        Returns:
            str: Kết quả thực thi các lệnh

        Note:
            - Sử dụng prompt đặc biệt để yêu cầu mô hình tạo ra các lệnh shell
            - Các lệnh được bọc trong thẻ <execute></execute>
            - Sử dụng Execute để thực thi các lệnh đã trích xuất
        """
