...
class OLLAMAPI:
    ...
    def _chat_completion(self, history: List[dict]) -> str:
        """
        Gửi yêu cầu chat completion đến API Ollama.

        Args:
            history (List[dict]): Lịch sử hội thoại

        Returns:
            str: Nội dung phản hồi từ mô hình
    ...
    def send_new_message(self, message: str, image_url: str = None):
        """
        Tạo một cuộc hội thoại mới và gửi tin nhắn đầu tiên.

        Args:
            message (str): Nội dung tin nhắn
            image_url (str, optional): URL của hình ảnh kèm theo

        Returns:
            tuple: (phản hồi từ mô hình, ID cuộc hội thoại mới)
        """
        # create a message
    ...
    def send_message(
        ...
        """
        Gửi tin nhắn trong một cuộc hội thoại hiện có.

        Args:
            message (str): Nội dung tin nhắn
            conversation_id (str): ID của cuộc hội thoại
            image_url (str, optional): URL của hình ảnh kèm theo
            debug_mode (bool): Chế độ debug để in thông tin chi tiết

        Returns:
            str: Phản hồi từ mô hình

        Note:
            - Tự động thử lại tối đa 2 lần khi có lỗi kết nối
            - Giữ lịch sử tin nhắn trong giới hạn history_length
        """
...
class OPENAI:
    ...
    def _chat_completion(self, history: List[dict]) -> str:
        """
        Gửi yêu cầu chat completion đến API OpenAI.
