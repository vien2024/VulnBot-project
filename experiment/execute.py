...
class Execute(BaseModel):
    ...
    def parse_response(self) -> list[str]:
        """
        Phân tích chuỗi instruction để trích xuất các lệnh trong thẻ <execute>.

        Returns:
            list[str]: Danh sách các lệnh đã được trích xuất và làm sạch

        Note:
            Sử dụng regex để tìm và làm sạch các lệnh trong thẻ <execute></execute>
        """
    ...
    def run(self) -> str:
        """
        Thực thi danh sách các lệnh và xử lý kết quả.

        Returns:
            str: Chuỗi kết quả chứa các lệnh và kết quả thực thi

        Note:
            - Xử lý các trường hợp đặc biệt như prompt nhập mật khẩu
            - Tự động gửi Ctrl+C trong một số trường hợp
            - Xử lý lỗi khi chưa thiết lập kết nối SSH
        """
