...
class OllamaChat(ABC):
    ...
    def _ensure_model_exists(self):
        ...
        try:
            models_on_server = self.client.list()["models"]
            if not any(m["name"] == self.model_name for m in models_on_server):
                print(
                    f"⚠️  Ollama: Model '{self.model_name}' không tìm thấy. Bắt đầu tải..."
                )
                # Bắt đầu quá trình pull, có hiển thị progress
                current_digest = ""
                for progress in self.client.pull(self.model_name, stream=True):
                    digest = progress.get("digest", "")
                    if digest != current_digest and current_digest != "":
                        # In một dòng mới khi digest thay đổi (tức là một layer mới)
                        print()
                    ...
                    if total > 0:
                        ...
                        # Ghi đè trên cùng một dòng để tạo hiệu ứng progress bar
            ...
            # Có thể raise lỗi ở đây để dừng hẳn chương trình nếu muốn
            raise e
