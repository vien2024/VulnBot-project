...
from rag.kb.repository.kb_repository import (
    add_kb_to_db,
    load_kb_from_db,
    list_kbs_from_db,
    kb_exists,
...
class KBService(ABC):
    ...
    def drop_kb(self):
        ...
        """
        try:
            # Lấy đường dẫn thư mục của KB
            kb_path = get_kb_path(self.kb_name)
            # Kiểm tra nếu thư mục tồn tại thì xóa nó đi
            if os.path.isdir(kb_path):
                shutil.rmtree(kb_path)
                logger.info(f"Successfully deleted knowledge base folder: {kb_path}")
        except Exception as e:
            logger.error(f"Failed to delete knowledge base folder {self.kb_name}: {e}")
            ...
            for doc in docs:
                try:
                    ...
                    if os.path.isabs(source):
                        # Convert self.doc_path to an absolute path before comparison
                        absolute_doc_path = Path(self.doc_path).resolve()
                        rel_path = Path(source).relative_to(absolute_doc_path)
...
class KBServiceFactory:
    ...
    def get_service_by_name(kb_name: str) -> KBService:
        ...
        if _ is None:  # kb not in db, just return None
            return None
        return KBServiceFactory.get_service(kb_name, vs_type, embed_model, _)
