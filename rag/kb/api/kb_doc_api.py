...
from rag.kb.utils.kb_utils import (
    validate_kb_name,
    KnowledgeFile,
    get_file_path,
    run_in_thread_pool,
...
def search_docs(
    ...
    metadata: dict = Body(
        {}, description="Filter based on metadata, only supports one level key"
    ),
) -> List[Dict]:
...
def _save_files_in_thread(
    ...
    def save_file(file: UploadFile, knowledge_base_name: str, override: bool) -> dict:
        ...
        try:
            ...
            return dict(
                code=200, msg=f"Successfully uploaded file {filename}", data=data
            )
...
def upload_docs(
    files: List[UploadFile] = File(
        ..., description="Upload files, support multiple files"
    ),
    knowledge_base_name: str = Form(
        ..., description="Knowledge base name", examples=["samples"]
    ),
    override: bool = Form(False, description="Override existing files"),
    to_vector_store: bool = Form(
        True, description="Whether to vectorize after uploading files"
    ),
    chunk_size: int = Form(
        Configs.kb_config.chunk_size,
        description="Maximum length of a single text segment in the knowledge base",
    ),
    chunk_overlap: int = Form(
        Configs.kb_config.overlap_size,
        description="Overlap length of adjacent text segments in the knowledge base",
    ),
    docs: str = Form(
        "", description="Custom docs, need to be converted to json string"
    ),
    not_refresh_vs_cache: bool = Form(
        False, description="Do not save vector store (for FAISS)"
    ),
) -> BaseResponse:
    """
    ...
    return BaseResponse(
        code=200,
        msg="Successfully uploaded and vectorized files",
...
def update_docs(
    ...
    file_names: List[str] = Body(
        ...,
        description="File names, supports multiple files",
        examples=[["file_name1", "text.txt"]],
    ),
    chunk_size: int = Body(
        Configs.kb_config.chunk_size,
        description="Maximum length of a single text segment in the knowledge base",
    ...
    override_custom_docs: bool = Body(
        False, description="Override previous custom docs"
    ),
    docs: str = Body(
        "", description="Custom docs, need to be converted to json string"
    ),
    not_refresh_vs_cache: bool = Body(
        False, description="Do not save vector store (for FAISS)"
    ),
) -> BaseResponse:
    """
    ...
    return BaseResponse(
        code=200,
        msg=f"Successfully updated documents",
        data={"failed_files": failed_files},
    )
...
def delete_docs_by_id(
    knowledge_base_name: str = Body(..., examples=["samples"]),
    doc_ids: List[str] = Body(..., examples=[["doc_id_1", "doc_id_2"]]),
) -> BaseResponse:
    """
    New function to delete documents by their specific IDs.
    """
    if not validate_kb_name(knowledge_base_name):
        return BaseResponse(code=403, msg="Don't attack me")
    ...
    try:
        if kb.del_doc_by_ids(doc_ids):
            return BaseResponse(
                code=200, msg=f"Successfully deleted {len(doc_ids)} documents."
            )
        else:
            return BaseResponse(code=500, msg="Failed to delete documents.")
    except Exception as e:
        logger.error(f"Error deleting docs by id: {e}")
        return BaseResponse(code=500, msg=f"An error occurred: {e}")
...
def update_info(
    ...
    kb_info: str = Body(
        ...
        examples=["This is a knowledge base"],
    ),
):
    if not validate_kb_name(knowledge_base_name):
        return BaseResponse(code=403, msg="Don't attack me")
    ...
    return BaseResponse(
        code=200,
        msg=f"Successfully updated knowledge base info",
