...
def delete_docs_from_db(
    ...
    """
    docs = list_docs_from_db(session, kb_name=kb_name, file_name=file_name)
    ...
    # session.commit() is handled by the decorator
    return docs


@with_session
def delete_docs_from_db_by_ids(session, kb_name: str, doc_ids: List[str]):
    """
    Deletes document records from the database based on a list of document IDs.
    """
    session.query(FileDocModel).filter(
        FileDocModel.kb_name.ilike(kb_name), FileDocModel.doc_id.in_(doc_ids)
...
def add_docs_to_db(session, kb_name: str, file_name: str, doc_infos: List[Dict]):
    ...
    if doc_infos is None:
        print(
            "Warning: doc_infos parameter in server.db.repository.knowledge_file_repository.add_docs_to_db is None"
        )
        return False
    for d in doc_infos:
        obj = FileDocModel(
...
def delete_file_from_db(session, kb_file: KnowledgeFile):
    ...
    if existing_file:
        ...
        # session.commit() is handled by decorator
