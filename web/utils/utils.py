...
class ApiRequest:
            ...
            try:
                async with response as r:
                    ...
                    async for chunk in r.aiter_text(None):
                        if (
                            not chunk
                        ):  # fastchat api yields empty bytes on start and end
                            continue
                        if as_json:
                            try:
    ...
    def search_kb_docs(
        ...
        score_threshold: float = 1.0,  # Increase threshold to get all docs for a file
        file_name: str = "",
        metadata: dict = {},
    ) -> List:
        """
    ...
    def delete_kb_docs_by_id(
        self,
        knowledge_base_name: str,
        doc_ids: List[str],
    ):
        """
        Corresponds to the new /knowledge_base/delete_docs_by_id endpoint.
        """
        data = {
            "knowledge_base_name": knowledge_base_name,
            "doc_ids": doc_ids,
        ...
        response = self.post(
            "/knowledge_base/delete_docs_by_id",
            json_data=data,
        )
        return self._get_response_value(response, as_json=True)
    ...
    def kb_chat(self, query: str, knowledge_base_name: str):
        """
        Corresponds to the /chat/kb_chat endpoint.
        """
        data = {
            "query": query,
            "knowledge_base_name": knowledge_base_name,
...
def check_error_msg(data: Union[str, dict, list], key: str = "msg") -> str:
    """
    Return an error message if an error occurred when requesting the API.
    """
    if isinstance(data, dict):
        if key in data and data.get("code") != 200:
            return data[key]
